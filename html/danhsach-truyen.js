...
const flavoursContainer = document.getElementById('menu-top');
const flavoursScrollWidth = flavoursContainer.clientWidth;

window.addEventListener('load', () => {
    let fistTime = true;
    let lastTime = false;
    self.setInterval(() => {
        console.log(flavoursContainer.scrollLeft,flavoursScrollWidth);
        if (flavoursContainer.scrollLeft <= flavoursScrollWidth) {
            if (fistTime) {
                fistTime = !fistTime;
                lastTime = !lastTime;
                flavoursContainer.scroll({
                    left: flavoursContainer.scrollLeft + 200,
                    behavior: "smooth"
                });
            } else {
                flavoursContainer.scroll({
                    left: flavoursContainer.scrollLeft + 100,
                    behavior: "smooth"
                });
            }
        } else {
            ...
            if(lastTime){
                ...
                fistTime = !fistTime;
                flavoursContainer.scroll({
                    left: flavoursContainer.scrollLeft + 1000,
                    behavior: "smooth"
                });
            }else {
                flavoursContainer.scroll({
